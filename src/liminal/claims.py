...
def extract_claims(
    text: str, source_url: str, observed_at: datetime, minimum_length: int = 20
) -> list[Claim]:
    claims = []
    for sentence in _sentences(text):
        if len(sentence) < minimum_length:
            continue
        claim_id = (
            "claim_" + content_hash(source_url + "|" + normalize_text(sentence))[:20]
        )
        confidence = (
            0.6
            if any(
        ...
        claims.append(
            Claim(
                ...
                evidence_hash=content_hash(sentence),
            )
        )
    return claims
...
def extract_entities(text: str) -> list[Entity]:
    ...
    candidates.extend(
        re.findall(
            r"\b[A-Z][A-Za-z0-9&.-]{2,}(?:\s+[A-Z][A-Za-z0-9&.-]{2,}){0,3}\b", text
        )
    )
