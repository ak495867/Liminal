...
def test_collector_rejects_private_and_local_targets():
    for url in ("http://localhost", "http://127.0.0.1", "http://192.168.1.1", "file:///tmp/evidence"):
...
def test_python_source_has_no_comment_lines():
    root = Path(__file__).parents[1]
    paths = list((root / "src").rglob("*.py")) + list((root / "tests").rglob("*.py")) + list((root / "examples").rglob("*.py"))
    assert paths
    for path in paths:
